 ...
 * - SYSTEM_INSTRUCTIONS - The core system instructions provided to the AI model.
 */
...
const generateImageTool = ai.defineTool(
  ...
  async (input) => { // Note: input here is specific to the tool, not the main flow's input
    console.log(`Generating image with prompt: ${input.imagePrompt}`);
    // For now, generateImageTool does not directly use the temperature from the main flow.
    // It will use the default temperature of the gemini-2.0-flash-exp model or a globally set one.
    // If more granular control is needed, temperature could be added to this tool's inputSchema.
...
const searchAndSummarizePrompt = ai.definePrompt({
  ...
  input: {schema: SearchAndSummarizeInputSchema}, // This input is now SearchAndSummarizeInputSchema
  output: {schema: SearchAndSummarizeOutputSchema},
  tools: [internetSearchTool, generateImageTool],
  prompt: (input: SearchAndSummarizeInput): Array<any> => { // input is of type SearchAndSummarizeInput
...
Assistant's Response (Remember to structure as a valid JSON object with a "summary" string (in the detected language), and optionally "imageUrl" string, following all rules above):`
  ...
  config: (input: SearchAndSummarizeInput) => { // config can be a function of the prompt's input
    const configOptions: { temperature?: number } = {};
...
const searchAndSummarizeFlow = ai.defineFlow(
  ...
  async (input: SearchAndSummarizeInput): Promise<SearchAndSummarizeOutput> => {
    try {
      // The input already contains the temperature if provided
